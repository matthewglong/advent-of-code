...
you're not climbing a mountain. As the circle of Snow Island recedes below you, an entir
...
has two lists of numbers separated by a vertical bar (|): a list of winning numbers and then a
list of numbers you have. You organize the information into a table (your puzzle input).
...
def parse_line(line):
    raw_split = line.split(': ')[1].split(' | ')
    clean_line = map(chunk_nums, raw_split)
    return clean_line
...
for line in raw_lines:
    winning_card, actual_card = parse_line(line)
    wins = [num for num in actual_card if num in winning_card]
    num_wins = len(wins)
    if num_wins:
        points = 2**(num_wins-1)
        total_points += points
...
print(f"There are a total of {total_points} points")
...
of the same cards that the original card 10 won: cards 11, 12, 13, 14, and 15. This process repeats
until none of the copies cause you to win any more cards. (Cards will never make you copy a card past
...
- Your four instances of card 3 (one original and three copies) have two matching numbers, so you winfour
...
- Once all of the originals and copies have been processed, you end up with 1 instance of card 1, 2
    ...
    instance of card 6. In total, this example pile of scratchcards causes you to ultimately have 30 scratchcards!
...
for i, line in enumerate(raw_lines):
    if i in cards:
        ...
        cards[i] = {'quantity': 1}
    winning, actual = parse_line(line)
    wins = len([num for num in actual if num in winning])
    cards[i]['wins'] = wins
    for _ in range(cards[i]['quantity']):
        for extra_card in range(wins):
            bonus_i = i + extra_card + 1
            if bonus_i in cards:
                cards[bonus_i]['quantity'] += 1
            else:
                cards[bonus_i] = {'quantity': 1}


total_cards = sum([card['quantity'] for card in cards.values()])
