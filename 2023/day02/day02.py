...
fluffy pile of leaves. It's quite cold, but you don't see much snow. An Elf runs over to greet you.
...
one point the Elf showed you 20 red cubes at once; similarly, game 4 would also have been
...
For each game, find the minimum set of cubes that must have been present. What is the sum of the power
of these sets?
"""
...
def parse_game(raw_game):
    game_name, game_data = raw_game.split(': ')
    game_number = int(game_name[5:])
    clean_game = []
    for turn in game_data.split('; '):
        clean_turn = {}
        for draw in turn.split(', '):
            quantity, color = draw.split(' ')
            clean_turn[color] = int(quantity)
        clean_game.append(clean_turn)

    return game_number, clean_game


def is_valid_game(clean_game, criteria):
    for turn in clean_game:
        for color, quantity in turn.items():
            if quantity > criteria[color]:
                return False
    else:
        return True


def minimum_power(clean_game):
    color_max = {}
    for turn in clean_game:
        for color, quantity in turn.items():
            if (color not in color_max) or (quantity > color_max[color]):
                color_max[color] = quantity
    power = 1
    for max_quantity in color_max.values():
        power *= max_quantity
    return power
...
for raw_game in raw_games:
    game_id, clean_game = parse_game(raw_game)
    if is_valid_game(clean_game, criteria):
        game_id_sum += game_id
    min_total_power += minimum_power(clean_game)

print(f"The sum total of all valid game ids is: {game_id_sum}")
print(f"The sum total of all minimum powers is: {min_total_power}")
