...
the familiar halls of a North Pole research wing.
...
Suddenly, you find yourself in an unfamiliar room! The room has no doors; the only way
out is the teleporter. Unfortunately, the teleporter seems to be leaking magic smoke.

Since this is a teleporter lab, there are lots of spare parts, manuals, and diagnostic
equipment lying around. After connecting one of the diagnostic tools, it helpfully
displays error code 0H-N0, which apparently means that there's an issue with one of the
tachyon manifolds.
...
(^), the beam is stopped; instead, a new tachyon beam continues from the immediate left
...
.^.^.^.^.^...^.
...............
In this example, the incoming tachyon beam (|) extends downward from S until it reaches
the first splitter:
...
.^.^.^.^.^...^.
...............
At this point, the two splitters create a total of only three tachyon beams, since they
are both dumping tachyons into the same place between them:
...
of the tachyon manifold. In this example, a tachyon beam is split a total of 21 times.
...
class TachyonManifold:
    def __init__(self):
        ...
        self.manifold_diagram = data
        self.width = len(data[0])
        self.height = len(data)

    def simulate_splits(self):
        simulation = self.manifold_diagram.copy()
        num_splits = 0
        beam_indexes = set()
        beam_indexes.add(simulation[0].index("S"))
    
        for y in range(1, self.height):
            new_beam_indexes = beam_indexes.copy()
            for x in beam_indexes:
                if simulation[y][x] == SPLIT:
                    num_splits += 1
                    left_side = x-1
                    right_side = x+1
                    if (left_side >= 0):
                        simulation[y][left_side] = BEAM
                        new_beam_indexes.add(left_side)
                    if (right_side < self.width):
                        simulation[y][right_side] = BEAM
                        new_beam_indexes.add(right_side)
                    new_beam_indexes.discard(x)
                else:
                    simulation[y][x] = BEAM
